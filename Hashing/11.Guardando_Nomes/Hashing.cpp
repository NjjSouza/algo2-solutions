...
int hash_insert(dado t[], int m, int k, string info)
{
    int i;
    int j;
...
int hash_search(dado t[], int m, int k, string info)
{
    int i;
    int j;
    ...
    do
    {
        ...
        if (t[j].status == 1 &&
        t[j].k == k && // alterada para verificar status
        t[j].info == info) // alterada para evitar confundir duas strings de mesma soma ASCII
    {
        return j;
    }
...
int hash_remove(dado t[], int m, int k, string info)
{
    int j;
